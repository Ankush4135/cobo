...
func _ready():
	...
	var level = 1
	while LevelManager.level_info[int(level)]["disabled"] == false: # check last unlocked level and the grab that button level
		level += 1
	get_node("Set/ScrollContainer/HBoxContainer/Button_" + str(level - 1) + "/Level_Button").grab_focus()
